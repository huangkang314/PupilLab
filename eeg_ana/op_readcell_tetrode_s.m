function [ts_T,wave1elec,waveTelec,chan_cell,firewave,LR_T,IsoD_T,Trodalness,info] = op_readcell_tetrode_s(filename,chan,long)
%--------------预定义变量-----------------------%
Fs = 40000;
first = long(1)/10^3/10^3*Fs;
last = long(2)/10^3/10^3*Fs;
cc = 1;
ts_T = [];    % cell 结构
wave1elec = [];
waveTelec = [];
...
for i_chan = 1 : size(DSPchan,1);    % 每一个通道
    ...
    for i_cell = 1:size(cellbegin,1)/4
        ...
        Feaspike_Norm(:,1) = F_Energy(cat(1,aa(:,first : last),bb(:,first : last)));
